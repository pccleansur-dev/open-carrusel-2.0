import { mutateData, readDataSafe } from "./data-store";
import { generateId, now } from "../utils";
import type { Carousel, CarouselsData, Slide, AspectRatio, ReferenceImage } from "@/types/carousel";
import { MAX_SLIDES, MAX_VERSIONS } from "@/types/carousel";

const FILE = "carousels.json";
const EMPTY_CAROUSELS_DATA: CarouselsData = { carousels: [] };

async function load(): Promise<CarouselsData> {
  return readDataSafe<CarouselsData>(FILE, EMPTY_CAROUSELS_DATA);
}

export async function listCarousels(): Promise<Carousel[]> {
  const data = await load();
  return data.carousels.filter((c) => !c.isTemplate);
}

export async function getCarousel(id: string): Promise<Carousel | null> {
  const data = await load();
  return data.carousels.find((c) => c.id === id) ?? null;
}

export async function createCarousel(
  name: string,
  aspectRatio: AspectRatio
): Promise<Carousel> {
  return mutateData(FILE, EMPTY_CAROUSELS_DATA, (data) => {
    const timestamp = now();
    const carousel: Carousel = {
      id: generateId(),
      name,
      aspectRatio,
      slides: [],
      referenceImages: [],
      chatSessionId: null,
      scheduledAt: null,
      postedAt: null,
      publishedPostId: null,
      publishedPostUrl: null,
      isTemplate: false,
      tags: [],
      createdAt: timestamp,
      updatedAt: timestamp,
    };
    data.carousels.push(carousel);
    return { result: carousel };
  });
}

export async function listScheduledCarousels(): Promise<Carousel[]> {
  const data = await load();
  const now = new Date().toISOString();
  return data.carousels.filter(
    (c) => !c.isTemplate && c.scheduledAt != null && c.scheduledAt <= now
  );
}

export async function updateCarousel(
  id: string,
  updates: Partial<
    Pick<
      Carousel,
      | "name"
      | "aspectRatio"
      | "tags"
      | "chatSessionId"
      | "caption"
      | "hashtags"
      | "scheduledAt"
      | "postedAt"
      | "publishedPostId"
      | "publishedPostUrl"
    >
  >
): Promise<Carousel | null> {
  return mutateData(FILE, EMPTY_CAROUSELS_DATA, (data) => {
    const idx = data.carousels.findIndex((c) => c.id === id);
    if (idx === -1) return { result: null, changed: false };
    Object.assign(data.carousels[idx], updates, { updatedAt: now() });
    return { result: data.carousels[idx] };
  });
}

export async function duplicateCarousel(id: string): Promise<Carousel | null> {
  return mutateData(FILE, EMPTY_CAROUSELS_DATA, (data) => {
    const source = data.carousels.find((c) => c.id === id);
    if (!source) return { result: null, changed: false };

    const timestamp = now();
    const duplicate: Carousel = {
      ...source,
      id: generateId(),
      name: `${source.name} (copy)`,
      slides: source.slides.map((s) => ({
        ...s,
        id: generateId(),
        previousVersions: [],
      })),
      referenceImages: [...(source.referenceImages || [])],
      chatSessionId: null,
      scheduledAt: null,
      postedAt: null,
      publishedPostId: null,
      publishedPostUrl: null,
      isTemplate: false,
      createdAt: timestamp,
      updatedAt: timestamp,
    };

    data.carousels.push(duplicate);
    return { result: duplicate };
  });
}

export async function deleteCarousel(id: string): Promise<boolean> {
  return mutateData(FILE, EMPTY_CAROUSELS_DATA, (data) => {
    const idx = data.carousels.findIndex((c) => c.id === id);
    if (idx === -1) return { result: false, changed: false };
    data.carousels.splice(idx, 1);
    return { result: true };
  });
}

// --- Slide operations ---

export async function addSlide(
  carouselId: string,
  html: string,
  notes = ""
): Promise<Slide | null> {
  return mutateData(FILE, EMPTY_CAROUSELS_DATA, (data) => {
    const carousel = data.carousels.find((c) => c.id === carouselId);
    if (!carousel || carousel.slides.length >= MAX_SLIDES) {
      return { result: null, changed: false };
    }

    const slide: Slide = {
      id: generateId(),
      html,
      previousVersions: [],
      order: carousel.slides.length,
      notes,
    };
    carousel.slides.push(slide);
    carousel.updatedAt = now();
    return { result: slide };
  });
}

export async function updateSlide(
  carouselId: string,
  slideId: string,
  updates: Partial<Pick<Slide, "html" | "notes">>
): Promise<Slide | null> {
  return mutateData(FILE, EMPTY_CAROUSELS_DATA, (data) => {
    const carousel = data.carousels.find((c) => c.id === carouselId);
    if (!carousel) return { result: null, changed: false };
    const slide = carousel.slides.find((s) => s.id === slideId);
    if (!slide) return { result: null, changed: false };

    if (updates.html && updates.html !== slide.html) {
      slide.previousVersions.push(slide.html);
      if (slide.previousVersions.length > MAX_VERSIONS) {
        slide.previousVersions.shift();
      }
    }

    Object.assign(slide, updates);
    carousel.updatedAt = now();
    return { result: slide };
  });
}

export async function deleteSlide(
  carouselId: string,
  slideId: string
): Promise<boolean> {
  return mutateData(FILE, EMPTY_CAROUSELS_DATA, (data) => {
    const carousel = data.carousels.find((c) => c.id === carouselId);
    if (!carousel) return { result: false, changed: false };
    const idx = carousel.slides.findIndex((s) => s.id === slideId);
    if (idx === -1) return { result: false, changed: false };

    carousel.slides.splice(idx, 1);
    carousel.slides.forEach((s, i) => {
      s.order = i;
    });
    carousel.updatedAt = now();
    return { result: true };
  });
}

export async function reorderSlides(
  carouselId: string,
  slideIds: string[]
): Promise<boolean> {
  return mutateData(FILE, EMPTY_CAROUSELS_DATA, (data) => {
    const carousel = data.carousels.find((c) => c.id === carouselId);
    if (!carousel) return { result: false, changed: false };

    const slideMap = new Map(carousel.slides.map((s) => [s.id, s]));
    const reordered: Slide[] = [];
    for (const id of slideIds) {
      const slide = slideMap.get(id);
      if (!slide) return { result: false, changed: false };
      slide.order = reordered.length;
      reordered.push(slide);
    }
    carousel.slides = reordered;
    carousel.updatedAt = now();
    return { result: true };
  });
}

export async function undoSlide(
  carouselId: string,
  slideId: string
): Promise<Slide | null> {
  return mutateData(FILE, EMPTY_CAROUSELS_DATA, (data) => {
    const carousel = data.carousels.find((c) => c.id === carouselId);
    if (!carousel) return { result: null, changed: false };
    const slide = carousel.slides.find((s) => s.id === slideId);
    if (!slide || slide.previousVersions.length === 0) {
      return { result: null, changed: false };
    }

    const previousHtml = slide.previousVersions.pop()!;
    slide.html = previousHtml;
    carousel.updatedAt = now();
    return { result: slide };
  });
}

// --- Reference images ---

export async function addReferenceImage(
  carouselId: string,
  image: ReferenceImage
): Promise<ReferenceImage | null> {
  return mutateData(FILE, EMPTY_CAROUSELS_DATA, (data) => {
    const carousel = data.carousels.find((c) => c.id === carouselId);
    if (!carousel) return { result: null, changed: false };

    if (!carousel.referenceImages) carousel.referenceImages = [];
    carousel.referenceImages.push(image);
    carousel.updatedAt = now();
    return { result: image };
  });
}

export async function removeReferenceImage(
  carouselId: string,
  imageId: string
): Promise<boolean> {
  return mutateData(FILE, EMPTY_CAROUSELS_DATA, (data) => {
    const carousel = data.carousels.find((c) => c.id === carouselId);
    if (!carousel || !carousel.referenceImages) {
      return { result: false, changed: false };
    }

    const idx = carousel.referenceImages.findIndex((img) => img.id === imageId);
    if (idx === -1) return { result: false, changed: false };

    carousel.referenceImages.splice(idx, 1);
    carousel.updatedAt = now();
    return { result: true };
  });
}
