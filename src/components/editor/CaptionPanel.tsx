...
export function CaptionPanel({
  ...
  const saveDraft = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/carousels/${carouselId}/caption`, {
        method: "PUT",
      ...
      if (!response.ok) {
        throw new Error("No se pudo guardar el caption");
      }
  ...
  const regenerate = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("No se pudo regenerar el caption");
      }
        ...
        <div className="px-4 pb-3 space-y-3">
          ...
          <div>
            ...
            {editing ? (
              ...
              <p className="rounded-md bg-muted p-2 text-xs text-foreground whitespace-pre-wrap">
                {caption}
              </p>
            )}
          </div>
